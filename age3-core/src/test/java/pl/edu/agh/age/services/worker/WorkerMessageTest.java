...
public final class WorkerMessageTest {
	...
	@Test(expected = IllegalArgumentException.class) public void testBroadcastMessage_wrongType() {
		new WorkerMessage<>(WorkerMessage.Type.UNICAST_CONTROL, PAYLOAD);
	}

	@Test public void testUnicastMessage_correct() {
		final WorkerMessage message = new WorkerMessage(WorkerMessage.Type.LOAD_CONFIGURATION,
		                                                ImmutableSet.of(RECEIVER), PAYLOAD);
	...
	@Test(expected = IllegalArgumentException.class) public void testUnicastMessage_emptyRecipients() {
