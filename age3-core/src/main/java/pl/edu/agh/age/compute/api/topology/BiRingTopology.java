 ...
 * - right
 */
public final class BiRingTopology<T extends Serializable> implements Topology<T> {
	...
	@Override public Graph<T, AnnotatedEdge> apply(final Set<T> identities) {
		...
		StreamEx.of(sorted).forPairs((t1, t2) -> {
			graph.addEdge(t1, t2, new AnnotatedEdge("right"));
			graph.addEdge(t2, t1, new AnnotatedEdge("left"));
		});
		...
		if (Objects.equals(first, last)) {
			graph.addEdge(last, first, new AnnotatedEdge("right", "left"));
		} else {
			graph.addEdge(last, first, new AnnotatedEdge("right"));
			graph.addEdge(first, last, new AnnotatedEdge("left"));
		}
