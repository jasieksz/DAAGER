...
class MetricsController @Inject()(
  ...
  private val supervisor = system.actorOf(
    Props(
  ...
  def startPulling(): Action[StartRequest] =
    Action(validateJson[StartRequest]) { request =>
  ...
  def getStatuses: Action[AnyContent] = Action.async {
    Patterns
      .ask(supervisor, MetricsSupervisor.GetStatus, timeout)
  ...
  def stopPuller(): Action[MetricsSupervisor.Stop] =
    Action(validateJson[MetricsSupervisor.Stop]) { request =>
      supervisor ! request.body
      Ok("")
    }
