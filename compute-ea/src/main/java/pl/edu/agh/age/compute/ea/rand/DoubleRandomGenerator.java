/*
 * Copyright (C) 2016-2018 Intelligent Information Systems Group.
 *
 * This file is part of AgE.
 *
 * AgE is free software: you can redistribute it and/or modify
 * it under the terms of the GNU General Public License as published by
 * the Free Software Foundation, either version 3 of the License, or
 * (at your option) any later version.
 *
 * AgE is distributed in the hope that it will be useful,
 * but WITHOUT ANY WARRANTY; without even the implied warranty of
 * MERCHANTABILITY or FITNESS FOR A PARTICULAR PURPOSE.  See the
 * GNU General Public License for more details.
 *
 * You should have received a copy of the GNU General Public License
 * along with AgE.  If not, see <http://www.gnu.org/licenses/>.
 */

package pl.edu.agh.age.compute.ea.rand;

/**
 * An interface for generating random double values.
 *
 * The contract of this interface is that values returned by {@code nextDouble()} must be included between those
 * returned by {@code lowerDouble()} and {@code upperDouble()}.
 *
 * Apart from that, those values can follow an arbitrary distribution, i.e. does not have to be uniformly distributed.
 */
public interface DoubleRandomGenerator {

	/**
	 * Returns a random double value arbitrarily distributed in the range
	 * {@code [lowerDouble(), upperDouble()]}.
	 *
	 * @return A random value.
	 */
	double nextDouble();

	/**
	 * Specifies the lower bound of the values that can be returned by {@code nextDouble()}.
	 *
	 * @return This generator's lower bound.
	 */
	double lowerDouble();

	/**
	 * Specifies the upper bound of the values that can be returned by {@code nextDouble()}.
	 *
	 * @return This generator's upper bound.
	 */
	double upperDouble();
}
