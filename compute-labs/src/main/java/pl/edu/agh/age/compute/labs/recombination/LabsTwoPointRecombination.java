...
public final class LabsTwoPointRecombination implements Recombination<LabsSolution> {
	...
	@Override public LabsSolution recombine(final LabsSolution firstSolution, final LabsSolution secondSolution) {
		...
		final int indexA = 1 + rand.nextInt(length - 2);
		final int indexB = RandomUtils.nextInt(1, length - 1, indexA);

		final boolean[] resultSolution = new boolean[length];
		final int lowerBound = Math.min(indexA, indexB);
		final int higherBound = Math.max(indexA, indexB);
